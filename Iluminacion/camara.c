...
void matriz_inversa(camara *c){
    ...
    c->M[4] = c->Minv[1];
    c->M[8] = c->Minv[2];
    c->M[12] = (-1)*(c->Minv[12] * c->Minv[0] + c->Minv[13] * c->Minv[1] + c->Minv[14] * c->Minv[2]);

    c->M[1] = c->Minv[4];
    c->M[5] = c->Minv[5];
    c->M[9] = c->Minv[6];
    c->M[13] = (-1)*(c->Minv[12] * c->Minv[4] + c->Minv[13] * c->Minv[5] + c->Minv[14] * c->Minv[6]);

    c->M[2] = c->Minv[8];
    c->M[6] = c->Minv[9];
    c->M[10] = c->Minv[10];
    c->M[14] = (-1)*(c->Minv[12] * c->Minv[8] + c->Minv[13] * c->Minv[9] + c->Minv[14] * c->Minv[10]);
...
void modo_analisis(int x, int y){
    ...
    if(_selected_object != 0) {
        ...
        px = _selected_object->mptr->M[12] - _selected_camara->M[12];
        py = _selected_object->mptr->M[13] - _selected_camara->M[13];
        pz = _selected_object->mptr->M[14] - _selected_camara->M[14];
        ...
        glRotatef(10.0, -x, -y, 0);
        glTranslatef(0, 0, distance);
        glGetFloatv(GL_MODELVIEW_MATRIX, _selected_camara->Minv);
...
void centre_camera_to_obj(object3d *obj){
	vector3 camera_pos = (vector3) { 
		.x = _selected_camara->M[12], 
		.y = _selected_camara->M[13], 
		.z = _selected_camara->M[14] 
	};
    ...
    _selected_camara->M[0] = _selected_camara->Minv[0];
    _selected_camara->M[1] = _selected_camara->Minv[4];
    _selected_camara->M[2] = _selected_camara->Minv[8];
    _selected_camara->M[3] = 0;

    _selected_camara->M[4] = _selected_camara->Minv[1];
    _selected_camara->M[5] = _selected_camara->Minv[5];
    _selected_camara->M[6] = _selected_camara->Minv[9];
    _selected_camara->M[7] = 0;

    _selected_camara->M[8] = _selected_camara->Minv[2];
    _selected_camara->M[9] = _selected_camara->Minv[6];
    _selected_camara->M[10] = _selected_camara->Minv[10];
    _selected_camara->M[11] = 0;

    _selected_camara->M[12] = camera_pos.x;
    _selected_camara->M[13] = camera_pos.y;
    _selected_camara->M[14] = camera_pos.z;
    _selected_camara->M[15] = 1;
